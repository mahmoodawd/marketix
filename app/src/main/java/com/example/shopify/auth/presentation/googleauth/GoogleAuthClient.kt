...
class GoogleAuthClient(private val context: Context, private val oneTapClient: SignInClient) {
    ...
    suspend fun signInUsingIntent(intent: Intent): SignInResult {
        ...
        return try {
            ...
            SignInResult(data = user?.run {
                UserData(userId = uid, userName = displayName, avatarUrl = photoUrl?.toString())
            }, errorMessage = null)
        } catch (e: Exception) {
            e.printStackTrace()
            if (e is CancellationException) throw e
    ...
    fun getUserSignedIn(): UserData? = auth.currentUser?.run {
        UserData(
            userId = uid,
